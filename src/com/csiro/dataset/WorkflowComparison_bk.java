...
public class WorkflowComparison_bk {
	...
	public boolean compareWorkflow(Workflow w1, Workflow w2) throws JSONException {
		...
		//boolean version = compareVersion(w1.getVersionId(), w2.getVersionId());
		HashMap<String, ArrayList<String>> pDifference = compareProgramList(w1.getPrograms(), w2.getPrograms());
		...
		if(id == false) {
			...
			System.out.println("Version is changed ! ");
			_model.add(rp.generateRenameRecords(w1, w2, "version"));
		} */
	...
	public HashMap<String, ArrayList<String>> compareProgramList(ArrayList<ProgramBean> plist1, ArrayList<ProgramBean> plist2) throws JSONException {
		//boolean comparison = false; 
		...
		ArrayList<String> w1Samew2 = new ArrayList<String>();
			...
			for(int i=0; i<plist1.size(); i++) {
				...
				//	Model _proModel = compareProgram(prog1, prog2);
					...
					//}
					if(compareProgram(prog1, prog2)==true) {
			...
			for(int i=0; i<plist2.size(); i++) {
				...
				if(w1Samew2.contains(prog1.getProgramId()) ) {
					...
					for(int j=0; j<plist1.size(); j++) {
						...
						} else if(compareProgram(prog1, prog2) == true) {
	...
	public boolean compareProgram(ProgramBean p1, ProgramBean p2) throws JSONException {
		...
		if(p1.getType()=="workflow" && p2.getType()=="workflow") {
			if(p1.getProgramId().equalsIgnoreCase(p2.getProgramId()) ){
				//System.out.println("***" + p1.getProgramId() + "	" + p2.getProgramId() + "***"  );
				comparison = compareWorkflow(p1.getWorkflow(), p2.getWorkflow()); 
				//comparison=true;
