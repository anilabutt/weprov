...
public class WorkflowSpecProv{
	...
	public Model generateSpecificationRDF(Workflow workflow, Resource parentProgramInstance) {
		...
		String _workflowAuthor = workflow.getAuthor();
		...
		if( _workflowAuthor.equals("")) { 
			
		} else {
			Resource authorURI = _model.createResource(WeProvData.agent+_workflowAuthor );
			workflowInstance.addProperty(agent, authorURI);
			authorURI.addProperty(rdfTypeProperty, Agent);
			authorURI.addProperty(foafname, _model.createLiteral(_workflowAuthor));
		}
		...
		if (workflow.getControllers().size()>0) {
			...
			for(int _controllerIdx=0; _controllerIdx < _controllerArr.size(); _controllerIdx++) {
				...
				//System.out.println(" Program " +_sourceProgram);
				...
				if(sourceProgramInstance.getURI().equalsIgnoreCase(WeProvData.program)) { 
				} else {
					sourceProgramInstance.addProperty(controlledBy, controllerInstance );
				}
				
				if(targetProgramInstance.getURI().equalsIgnoreCase(WeProvData.program)) { 
				} else {
