...
public class WorkflowComparison {
	...
	public Model getRevisionProvenance(Workflow w1, Workflow w2, String revId) throws JSONException {
		
	//	provModel.add(compareWorkflow(w1, w2)) ;	
		compareWorkflow(w1, w2, revId) ;
		System.out.println("Before Returning . .. .. ." + provModel.size() );
		return provModel;
	}

	public Model compareWorkflow(Workflow w1, Workflow w2, String revId) throws JSONException {
		WorkflowRevProv rp = new WorkflowRevProv();
		Model _model = ModelFactory.createDefaultModel();
		Resource _revision = _model.getResource(WeProvData.revision + revId+ "/" + w1.getWorkflowId() );
		...
		HashMap<String, ArrayList<String>> pDifference = compareProgramList(w1.getPrograms(), w2.getPrograms(), _revision.toString(), revId);
				...
				(controller.get("w1Diffw2").size() == 0 && controller.get("w2Diffw1").size() ==0) ) 
		{
			...
			//_model = rp.generateWorkflowRevisionRecords(w1, w2);
			_model.add(rp.generateRevisionRDF(w1, w2, revId));
		...
		System.out.println("Checking outputs ..............");
		 ...
		 //Update Controllers
				...
				System.out.println("Checking Controllers ..............");
					...
					System.out.println("--------------------- :" +  provModel.size());
					return _model;
	}
	...
	public boolean compareAuthor(String author1, String author2) {
		boolean comparison = false; 
		if (author1.equalsIgnoreCase(author2) || (author1=="" && author2=="") ){
			//System.out.println("Workflow authors are same ..... \t" + author1+ "\t" + author2);
			comparison = true;
		} else {
			//System.out.println("Workflow authors are different ..... \t" + author1+ "\t" + author2);
		}
		return comparison;
	}
	...
	public HashMap<String, ArrayList<String>> compareProgramList(ArrayList<ProgramBean> plist1, ArrayList<ProgramBean> plist2, String revisionId, String revId) throws JSONException {
		//boolean comparison = false; 
		...
		ArrayList<String> w1Samew2 = new ArrayList<String>();
			...
			for(int i=0; i<plist1.size(); i++) {
				...
				//	Model _proModel = compareProgram(prog1, prog2);
					...
					//}
					if(compareProgram(prog1, prog2, revisionId, revId)==true) {
			...
			for(int i=0; i<plist2.size(); i++) {
				...
				if(w1Samew2.contains(prog1.getProgramId()) ) {
					...
					for(int j=0; j<plist1.size(); j++) {
						...
						} else if(compareProgram(prog1, prog2, revisionId, revId) == true) {
	...
	public boolean compareProgram(ProgramBean p1, ProgramBean p2, String revisionId, String revId) throws JSONException {
		...
		if(p1.getType()=="workflow" && p2.getType()=="workflow") {
			if(p1.getProgramId().equalsIgnoreCase(p2.getProgramId()) ){
				//System.out.println("***" + p1.getProgramId() + "	" + p2.getProgramId() + "***"  );
				model = compareWorkflow(p1.getWorkflow(), p2.getWorkflow(), revId); 
